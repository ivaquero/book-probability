...
#tip[
  $D_("KL")(p||q)$散度可被解释为通过使用概率分布$q$来近似分布$p$而引入的不确定性。
]
...
Jensen-Shannon 散度（$D_("JS")$）是一种对称散度。显然，我们有

$
  0 < D_("JS")(p||q) ≤ ln(2)
$

通过在下列等式中将$p\/(p+q)$和$q\/(p+q)$都替换为 1.0，可以证明这一点

$
  D_("JS") (p||q) = 1 / 2 ∫ (p(x) ln((2 p(x)) / (p(x) + q(x))) + q(x) ln((2 q(x)) / (p(x) + q(x)))) dd(x)
$
