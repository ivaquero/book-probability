...
$n$个对象中，每个对象都被独立归入$k$个类别之一。一个物体被归入$j$个类别的概率为$p_j$，其中$p_j$为非负数，且$∑_(j=1)^k p_j = 1$。假设$X_1$是第 1 类中的物体数，$X_2$是第 2 类中的物体数，等等，这样$X_1 + … + X_p = n$，则$𝑿 = (X_1, …, X_p)$称为多项式分布，参数为 $n$和$𝐩 = (p_1, …, p_p)$。记作$𝑿 ∼ "Mult"_p (n, 𝐩)$。
...
$
  𝜮 = 𝑼 𝚲 𝑼^⊤ = (u_1, u_2, …, u_p) "diag"(λ_i)(u_1, u_2, …, u_p)^⊤ = ∑_(i=1)^p u_i λ_i u_i^⊤
$

于是

$ 𝜮^(-1) = ∑_(i=1)^p u_i 1 / λ_i u_i^⊤ $

从而有

$ D_M (x) = ∑_(i=1)^p (𝒙 - 𝝁)^⊤ u_i 1 / λ_i u_i^⊤(𝒙 - 𝝁) $
令$y_i = (𝒙 - 𝝁)^⊤ u_i$，得

$ D_M (x) = ∑_(i=1)^p y_i^2 / λ_i $
