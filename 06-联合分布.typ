...
#figure(
  table(
    ...
    table.header([$Y$离散], [$Y$连续]),
    [$P(Y=y|X=x) = frac(P(X=x|Y=y) P(Y=y), P(X=x))$], [$f_Y (y|X=x) = frac(P(X=x|Y=y) f_Y (y), P(X=x))$],
    table.header([$X$离散], [$X$连续]),
...
#figure(
  table(
    ...
    stroke: table-three-line(rgb("000")),
    table.header([$Y$离散], [$Y$连续]),
    [$sum_y P(X=x|Y=y) P(Y=y)$], [$∫_(-∞)^(∞) P(X=x|Y=y) f_Y (y) dd(y)$],
    table.header([$X$离散], [$X$连续]),
...
$
  𝔼[|X - Y|] = ∫_(-∞)^(∞)∫_(-∞)^(∞)(|x - y|) 1 / sqrt(2pi) e^(-x^2 / 2) 1 / sqrt(2pi) e^(= y^2 / 2) dd(x, y)
$

又由矩母函数的结论，正态分布的和或差仍服从正态分布，可知$X - Y ∼ 𝒩(0, 2)$，于是有$X - Y = sqrt(2)Z$，得

$ 𝔼[Z] = sqrt(frac(2, pi)) ⇒ 𝔼[|X - Y|] = 2 / sqrt(pi) $
...
#definition[
  ...
  $
    f(𝒙) = frac(1, (2pi)^(p / 2) |𝜮|^(1 / 2)) exp(-1 / 2 (𝒙 - 𝝁)^⊤ 𝜮^(-1)(𝒙 - 𝝁))
  $
...
#theorem(title: ctext("样本均值与方差的独立性"))[
  设$X_1, … , X_n i.i.d. ∼ 𝒩(μ, σ^2)$，则$"Cov"(macron(X_n), S_n^2) = 0$
]

#theorem(title: ctext("积分布"))[
...
$
  f_(X, Y)(x, y) = frac(1, 2pi τ) exp(-frac(1, 2 τ^2)(x^2 + y^2 - 2ρ x y))
$

其中，$ρ ∈ (-1, 1)$为相关系数，$τ = sqrt(1 − ρ^2)$。

#theorem(title: ctext("和与差的独立性"))[
